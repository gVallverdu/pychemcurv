...
class VertexAtomTests(unittest.TestCase):
    ...
    def test_angular_defect(self):
        ...
        self.assertAlmostEqual(np.degrees(
            self.va_rand.angular_defect), 29.83127456)

    def test_spherical_curvature(self):
        self.assertAlmostEqual(
            self.va_sp3.spherical_curvature, 0.5128205128205)
    ...
    def test_improper(self):
        self.assertAlmostEqual(self.va_sp3.improper,
                               np.radians(-35.2643896828))
        self.assertAlmostEqual(self.va_sp2.improper, 0.)
        self.assertAlmostEqual(self.va_rand.improper,
                               np.radians(-30.021240733))
    ...
    def test_from_pyramid(self):
        ...
        self.assertTrue(np.allclose(self.va_sp3.a, va.a))
        self.assertTrue(np.allclose(self.va_sp3.star_a, va.star_a))

        # sp2 case
        va = VertexAtom.from_pyramid(self.l, self.theta_sp2, radians=True)
        self.assertTrue(np.allclose(self.va_sp2.a, va.a))
        self.assertTrue(np.allclose(self.va_sp2.star_a, va.star_a))
...
class HybridizationTests(unittest.TestCase):
    ...
    def test_coeffs(self):
        ...
        self.assertTrue(np.allclose(
            self.h.lambda_pi**2, [1, 0.78380312, 3 / 4]))
